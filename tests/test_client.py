...
def test_receive_message(mock_socket):
    ...
    assert str(exception.value) == "Error receiving message: Invalid message header"

    # Test receiving a message with incorrect length
    mock_socket.recv.side_effect = [b'0\n']
    with pytest.raises(Exception) as exception:
        receive_message(mock_socket)
    assert str(exception.value) == "Error receiving message: Error reading message from server"
