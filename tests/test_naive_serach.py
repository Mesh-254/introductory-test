...
import tempfile
import os
from search_algorithms.naive_search import naive_string_match
...
def test_string_exists():
    text = "hello\nworld\nfoo\nbar"
    pattern = "world"
    result, time_taken, current_time = naive_string_match(pattern, text)
    assert result == 'STRING EXISTS\n'
    assert isinstance(time_taken, float)
    assert isinstance(current_time, str)
    assert time.strptime(current_time, '%Y-%m-%d %H:%M:%S')
